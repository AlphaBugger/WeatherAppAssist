...
class WeeklyForecastView: UIView {
    ...
    private func setupView() {
        ...
        headerLabel.font = .systemFont(ofSize: 14, weight: .bold) // Customize the font to match your design
        ...
        NSLayoutConstraint.activate([
            collectionView.topAnchor.constraint(equalTo: headerLabel.bottomAnchor, constant: 0), // Space below the header label
    ...
    func updateForecasts(with weatherData: WeatherData) {
        // Process the list to group by day and extract min/max temperatures
        ...
        // Take only the first 5 elements to display the next 5 days forecast
//        forecasts = Array(forecasts.prefix(5))
        
        collectionView.reloadData()
    }
    
    // Helper function to map weather condition codes to SF Symbol names
    private func getSymbolName(from weatherConditionId: Int) -> String {
        // Match the condition codes with SF Symbols
        // This is a simplified example; match the conditions to the symbols you need
        switch weatherConditionId {
            ...
            default: return "questionmark.diamond.fill" // Unknown
        }
    }
}
